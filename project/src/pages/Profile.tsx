      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <Paper elevation={0} sx={{ borderRadius: 4, overflow: 'hidden', mb: 4, boxShadow: '0 10px 30px rgba(0,0,0,0.07)', border: theme => `1px solid ${theme.palette.divider}` }}>
            ...
            <Box sx={{ p: { xs: 3, md: 5 }, pt: { xs: 12, md: 14 }, display: 'flex', flexDirection: 'column', alignItems: { xs: 'center', md: 'flex-start' } }}>
              ...
              <Paper elevation={0} sx={{ mt: 4, p: 3, borderRadius: 2, bgcolor: theme => alpha(theme.palette.primary.main, 0.04), display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around', gap: 2, transition: 'box-shadow 0.2s', '&:hover': { boxShadow: '0 4px 24px rgba(80,0,200,0.10)' } }}>
                ...
                <Box sx={{ textAlign: 'center', px: 2 }}>
                  <Typography variant="h4" fontWeight="bold" color="primary.main">{sheets.reduce((acc, s) => acc + (s.downloads || 0), 0)}</Typography>
          ...
          >
            ...
            >
              <Tab label="Minhas Partituras" icon={<MusicNoteIcon />} iconPosition="start" />
              ...
              {loading ? (
                ...
                >
                  ...
                  <Typography variant="body2" color="text.secondary" mb={3}>
                    Compartilhe sua primeira partitura para que outros músicos possam acessá-la (ou não).
                  </Typography>
                  <Button
                    variant="contained"
                    color="primary"
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)}>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth margin="normal">
            ...
            <Select
              multiple
              value={editProfile.instruments}
              onChange={e => setEditProfile(p => ({ ...p, instruments: e.target.value as string[] }))}
            ...
            >
              {['Piano', 'Guitarra', 'Violino', 'Bateria', 'Voz', 'Outro'].map(instrument => (
          ...
          <FormControl fullWidth margin="normal">
            ...
            <Select
              multiple
              value={editProfile.musical_genres}
              onChange={e => setEditProfile(p => ({ ...p, musical_genres: e.target.value as string[] }))}
              renderValue={(selected) => (
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                  {(selected as string[]).map((value) => (
                    <Chip key={value} label={value} size="small" />
            ...
            >
              {['Rock', 'Pop', 'Jazz', 'Clássico', 'Blues', 'Hip Hop'].map(genre => (
          ...
          <FormControlLabel
            control={
              <Switch
                checked={editProfile.professional}
                onChange={e => setEditProfile(p => ({ ...p, professional: e.target.checked }))}
                color="primary"
              />
            }
          ...
          <FormControlLabel
            control={
              <Switch
                checked={editProfile.teaching}
                onChange={e => setEditProfile(p => ({ ...p, teaching: e.target.checked }))}
                color="primary"
              />
            }
            label="Oferece Aulas"
