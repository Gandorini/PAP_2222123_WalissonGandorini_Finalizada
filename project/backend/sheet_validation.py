    ...
    with TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            if best and best["tag_name"].lower() in allowed_tags:
                return JSONResponse({
                    "valid": True,
                    "message": f"Arquivo validado como '{best['tag_name']}' com probabilidade {best['probability']:.2f}.",
                    "prediction": best
                })
            else:
                return JSONResponse({
                    "valid": False,
                    "message": "O arquivo não foi reconhecido como partitura ou cifra com confiança suficiente.",
